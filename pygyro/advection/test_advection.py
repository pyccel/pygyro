...
def test_poloidalAdvectionImplicit(dt, v, xc, yc):
    ...
    polAdv = PoloidalAdvection(
        eta_vals, bsplines[::-1], constants, True, False, 1e-10)
...
def test_vParGradAligned():
    ...
    pG = ParallelGradient(grid.getSpline(1), grid.eta_grid,
                          grid.getLayout(grid.currentLayout), constants)
...
def test_Phi_deriv_dz(phiOrder, zOrder):
    ...
    for i in range(nconvpts):
        ...
        btheta = constants.iota(1)/constants.R0 / \
            np.sqrt(1+(constants.iota(1)/constants.R0)**2)
        ...
        pGrad = ParallelGradient(
            spline_theta, eta_grid, layout, constants, zOrder)

        approxGrad = np.empty([npts[2], npts[1]])
        pGrad.parallel_gradient(phiVals, 0, approxGrad)
        exactGrad = pg_dPhi(
            1, eta_grid[1][None, :], eta_grid[2][:, None], btheta, bz)

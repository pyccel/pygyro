...
def test_poloidalAdvection_constantAdv(initConditions, xc, yc):
    ...
    for i in range(nconvpts):
        ...
        phiVals[:] = Phi_adv(eta_vals[0], np.atleast_2d(
            eta_vals[1]).T, omega, xc, yc)
        ...
        finalPts = (np.ndarray([npts[1], npts[0]]),
                    np.ndarray([npts[1], npts[0]]))
        finalPts[0][:] = np.mod(np.arctan2(y, x), 2 * np.pi)
        finalPts[1][:] = np.sqrt(x * x + y * y)
        final_f_vals[:, :] = initConds(finalPts[1], finalPts[0])

        x = xc + (x0 - xc) * np.cos(omega * -dt) - \
            (y0 - yc) * np.sin(omega * -dt)
        y = yc + (x0 - xc) * np.sin(omega * -dt) + \
            (y0 - yc) * np.cos(omega * -dt)

        endPts = (np.ndarray([npts[1], npts[0]]),
                  np.ndarray([npts[1], npts[0]]))
...
def test_Phi_deriv_dtheta():
    ...
    print(str.format('{0:.2f}', l2[0] * 10**-mag2Order),
          "\\cdot 10^{", str.format('{0:n}', mag2Order), end=' ')
    print("}$ &       & $", end=' ')
    print(str.format('{0:.2f}', linf[0] * 10**-maginfOrder),
          "\\cdot 10^{", str.format('{0:n}', maginfOrder), end=' ')
    print("}$ &        \\\\")
    print("\\hline")
    for i in range(1, nconvpts):
        ...
        print(n, " & & $", end=' ')
        print(str.format('{0:.2f}', l2[i] * 10**-mag2Order),
              "\\cdot 10^{", str.format('{0:n}', mag2Order), end=' ')
        print("}$ & ", str.format('{0:.2f}', l2Order[i-1]), " & $", end=' ')
        print(str.format('{0:.2f}', linf[i] * 10**-maginfOrder),
              "\\cdot 10^{", str.format('{0:n}', maginfOrder), end=' ')
...
def test_Phi_deriv_dz():
    ...
    for i in range(nconvpts):
        ...
        pGrad = ParallelGradient(
            spline_theta, eta_grid, theLayout, constants, order=3)

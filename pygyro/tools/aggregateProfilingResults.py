...
for line in file:
    ...
    if (len(results)>0):
        div = results[0].split('/')
        results[0]=div[0]
        lines.append((results[5],[float(results[1]),float(results[3]),int(results[0])]))
...
for func,res in content.items():
    results.append([res[2]//nprocs,res[0]/nprocs,res[0]/res[2],res[1]/nprocs,res[1]/res[2],func])
...
for line in start_lines[:4]:
    file.write(line)
file.write('   ncalls    tottime    percall    cumtime    percall filename:lineno(function)\n')
for r in results:

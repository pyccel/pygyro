...
class SlicePlotterNd(object):
    ...
    def __init__(self, grid: Grid, xDimension: int, yDimension: int,
        ...
        self.oDims = list(set(range(len(grid.eta_grid))) -
                          set([*self.sDims, self.xDim, self.yDim]))
        ...
        self.selectDict = dict(
            zip([*self.oDims, *self.sDims], [*self.omitVals, *self.sliderVals]))
        ...
        if (self.rank == self.drawRank):
            ...
            self.buttons = [Button(self.button_axes[0], u"\u25b6"),
                            ...
                            CheckButtons(self.button_axes[2], [
                                         'Fix\n max/min\n colour'], [True]),
            ...
            for i in range(self.nSliders):
                ...
                slider.on_changed(
                    lambda val, dim=self.sDims[i]: self.updateVal(val, dim))

                slider.on_mem_changed(
                    lambda idx=i, dim=self.sDims[i]: self.updateMem(idx, dim))

                s_min, s_max = slider.reset_bounds()

                self.selectDict[slider.grid_dimension] = range(s_min, s_max+1)
                self.memory_requirements[self.sDims[i]
                                         ] = slider.get_next_n_poss_pts()
            ...
            self.completedRanks = [
                i == self.drawRank for i in range(self.comm_size)]
    ...
    def updateVal(self, value, dimension):
        ...
        if (self.playing):
            self.access_pattern[dimension] = value - \
                self.selectDict[dimension].start
        else:
    ...
    def play_pause(self, evt):
        ...
        if (self.playing):
            ...
            for i, d in enumerate(self.sDims):
                self.access_pattern[d] = self.sliders[i].idx - \
                    self.sliders[i].fix_min
    ...
    def fixBounds(self, evt):
        ...
        if ('vmin' in self.plotParams):
            ...
            # get max and min values of f to avoid colorbar jumps
            minimum = self.grid.getMin(
                self.drawRank, self.oDims, self.omitVals)
            maximum = self.grid.getMax(
                self.drawRank, self.oDims, self.omitVals)
    ...
    def getData(self):
        ...
        if (self.rank != self.drawRank):
            ...
            for i in range(self.nSliders):
                self.selectDict[self.selectInfo[i*3]
                                ] = range(self.selectInfo[i*3+1], self.selectInfo[i*2+2])
            self.grid.getBlockFromDict(
                self.selectDict, self.comm, self.drawRank)

        else:
            if (self.playing):
                for i, slider in enumerate(self.sliders):
                    s_min, s_max = slider.reset_bounds()
                    self.selectDict[slider.grid_dimension] = range(
                        s_min, s_max+1)
            ...
            layout, starts, mpi_data, theSlice = self.grid.getBlockFromDict(
                self.selectDict, self.comm, self.drawRank)

            split = np.split(theSlice, starts[1:])

            nprocs = [max([mpi_data[i][j] for i in range(
                len(mpi_data))])+1 for j in range(len(mpi_data[0]))]
                ...
                for k in self.selectDict.keys():
                    ...
                    if (j < n_ranks):
                        ...
                        if (isinstance(self.selectDict[k], int)):
                            ...
                            range_stop = min(
                                range_stop - start, layout.mpi_lengths(j)[ranks[j]])
                        ...
                        if (isinstance(self.selectDict[k], int)):
                            ...
                            layout_shape[j] = self.selectDict[k].stop - \
                                self.selectDict[k].start

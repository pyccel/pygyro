...
class DiagnosticCollector:
    def __init__( self, comm, saveStep: int, dt: float, distribFunc: Grid, phi: Grid ):
        ...
        self.l1class = l1(distribFunc.eta_grid,distribFunc.getLayout('v_parallel'))
    ...
    def reduce(self):
        ...
        self.comm.Reduce(self.diagnostics[3,:],self.l1Result,op=MPI.SUM, root=0)
        self.comm.Reduce(self.diagnostics[4,:],self.nPartResult,op=MPI.SUM, root=0)
        self.comm.Reduce(self.diagnostics[5,:],self.min_val,op=MPI.MIN, root=0)
        self.comm.Reduce(self.diagnostics[6,:],self.max_val,op=MPI.MAX, root=0)
        self.comm.Reduce(self.diagnostics[7,:],self.KE_val,op=MPI.SUM, root=0)
        if (self.rank == 0):

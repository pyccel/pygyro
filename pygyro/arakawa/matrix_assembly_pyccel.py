...
@pure
def lists_order2_periodic(N_theta: int, N_r: int,
                          phi: 'float[:]',
    ...
    Arakawa bracket of order 2 with periodic boundary conditions in
    r-direction.
    ...
    -----------
        ...
        row : array[int]
            array of length (N_r * N_theta * 9), will be filled with
...
@pure
def lists_order4_periodic(N_theta: int, N_r: int,
                          phi: 'float[:]',
                          row: 'int[:]', col: 'int[:]', data: 'float[:]'):
    """
    Fill the row, column, and data arrays which are needed for the
    creation of scipy sparse matrices in the assembly of the discrete
    Arakawa bracket of order 4 with periodic boundary conditions in
    r-direction.
    ...
    -----------
        ...
        row : array[int]
            array of length (N_r * N_theta * 17), will be filled with
            the row indices of the bracket matrix
    ...
    for ir in range(N_r):
        for it in range(N_theta):
            ...
            coef = phi[neighbour_index(-1, -1, ir, it, N_r, N_theta)] \
                - phi[neighbour_index(-1, 1, ir, it, N_r, N_theta)]
            row[k] = ii
            col[k] = neighbour_index(-2, 0, ir, it, N_r, N_theta)
            data[k] = - coef
            k += 1
            ...
            coef = -phi[neighbour_index(-1, -1, ir, it, N_r, N_theta)] \
                + phi[neighbour_index(1, -1, ir, it, N_r, N_theta)]
            row[k] = ii
            col[k] = neighbour_index(0, -2, ir, it, N_r, N_theta)
            data[k] = - coef
            k += 1
...
@pure
def lists_order2_dirichlet(N_theta: int, N_r: int,
                           phi: 'float[:]',
                           row: 'int[:]', col: 'int[:]', data: 'float[:]'):
    """
    Fill the row, column, and data arrays which are needed for the
    ...
    -----------
        ...
        row : array[int]
            array of length (N_theta * (N_r - 2) * 9 + 6 + 6)), will be filled with
            the row indices of the bracket matrix
...
@pure
def lists_order4_dirichlet(N_theta: int, N_r: int,
                           phi: 'float[:]',
                           row: 'int[:]', col: 'int[:]', data: 'float[:]'):
    """
    Fill the row, column, and data arrays which are needed for the
